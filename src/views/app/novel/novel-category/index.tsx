import { delNovelCategoryApi, getNovelCategoryPageApi, NovelCategoryModel } from "@/api/app/novel/novel-category";
...
const NovelCategory: React.FC = () => {
  ...
  const columns: ProColumns<NovelCategoryModel>[] = [
    ...
    },
    {
      ...
      render: (_, data) => (
        <Space>
          <HocAuth permission={["app:novel-category:edit"]}>
            <LoadingButton
              key="edit"
              type="link"
              size="small"
          ...
          <HocAuth permission={["app:novel-category:del"]}>
            <LoadingButton
              key="delete"
              type="link"
              size="small"
  ...
  const handleShowEditFormModal = (data: NovelCategoryModel, done: () => void) => {
  ...
  const handleDelete = (id: number, done: () => void) => {
    modal.confirm({
      ...
      content: "是否确认删除编号为 " + id + " 的分类？该分类下存在标签或书籍时将无法删除。",
      okText: "确认",
      cancelText: "取消",
      maskClosable: true,
      onCancel: () => {
      ...
      onOk: async () => {
        try {
          const { code, msg } = await delNovelCategoryApi([id!]);
  ...
  const toolBarRender = () => [
    <HocAuth permission={["app:novel-category:add"]}>
    ...
    <>
      <ProTable<NovelCategoryModel>
        ...
        request={async params => {
          const { data } = await getNovelCategoryPageApi(params);
          return formatDataForProTable<NovelCategoryModel>(data);
        }}
        columnsState={{
          persistenceKey: "use-pro-table-key-novel-category",
